...
function f5() {
    ...
    fetch(URL + '/api/27/gow/random-world', {
        ...
        .then(data => {
            console.log(data);
            document.querySelector('.out-5').innerHTML = data.result.governor;
        });
}
...
function f7(){
    ...
    const p1 = fetch(URL + '/api/27/random/random-number', {
        headers: requestHeaders
    }).then(r => {
        r.json()
    });

    const p2 = fetch(URL + '/api/27/random/random-string', {
        headers: requestHeaders
    }).then(r => {
        r.json()
    });
    ...
    Promise.all([p1, p2]).then(data => {
        console.log(data);
        document.querySelector('.out-71').innerHTML = data.result[0]['random-number'];
        document.querySelector('.out-72').innerHTML = data.result[1]['random-string'];
    });
}
...
const p1 = fetch(URL + '/api/27/sr/read/' + race1, {
    headers: requestHeaders
}).then(r => r.json());

const p2 = fetch(URL + '/api/27/sr/read?race=' + race2, {
    headers: requestHeaders
}).then(r => r.json());
...
Promise.all([p1, p2]).then(data => {
    ...
    data.forEach(raceData => {
        ...
        img.src = raceData.result.image;
            out8.appendChild(img);
    });
});
}
